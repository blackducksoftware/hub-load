...
function get_npss_url() {

  npss=$(cat $1 | awk '/Ephemeral mode signature scan URL/ {print}'  | awk '{print $12}')
  # echo "URL: $npss"  1>&2
...
# Defaults
#
BD_HUB_URL=${BD_HUB_URL:-https://jhjjj}
API_TOKEN=${API_TOKEN:-jkjkjkjkkj}
...
# whether the end index value reached the total number of files and if reached resetting it back to 0
  ...
  RANDOM=`date "+%s"`
    ...
    do
      ...
      url=$(get_npss_url $detect_log)
      echo "Elapsed time for scan ID ${url} was ${elapsed_time} seconds"
      rm $detect_log
      ((scans++))
    echo "looping"
  done
